...
> Detailed paper-grade findings live in the project's append-only scientific-findings log, with append-only audit trail — **493 numbered findings as of 2026-09-04**, each with its receipts, its pre-registration where it had one, and its refutations. Below are the *published* highlights that practitioners, paper reviewers and external benchmarkers should know about today.
>
...
The constructive successor is **pre-registered** (2026-09-03): retraining at operational extents — a fixed 10 m physical cell size instead of a frame notion, training crops drawn from large worlds whose horizon burns a *small* fraction, negatives included, and no per-window normalisation — with the corpus, model and serving deltas and **every validation bar frozen before any run**, including the prediction that would refute the approach and name its successor. Whatever it measures, the serving default changes under **no** outcome: pointing production at a new checkpoint is a separately gated operator decision under the holds above.

### 13. Property-based testing found float-domain defects the reference-parity suite could not

The FWI implementation passes fixture-point parity against the reference `cffdrs-R` implementation and always has. Replacing four-month-old placeholder test stubs with **real property-based (`hypothesis`) invariants** — 41 in the first wave alone — nevertheless produced failures on the first run of each of **four** successive waves, and every wave found the same defect family in a new place:
...
- **The final sweep — the FWI 2025 / next-generation indices — closed the last unswept corner with the lane's best operational-domain catch:** the vectorized grass-ROS pair emitted an invalid-value warning on **every calm-wind cell** (< 5 km/h), because `np.where` evaluates both branches and the discarded one raises a *negative* base to the fractional 0.844 power; and the grassland-moisture twins diverged on wild-but-finite weather, the scalar skipping the Magnus vapor-pressure ratio in the no-solar-heating case while the array computed it unconditionally — straight through the −237 °C pole.
...
Three by-products are more interesting than the repairs. **One drafted property was wrong and the code was right**: `BUI ≤ 0.8·DC` is not a bound, because the eq 28 correction deliberately pulls a low base BUI *up* toward DMC, exactly as `cffdrs-R` prescribes — a "physically obvious" bound must be derived from the equations, not assumed, and the wrong draft is kept in the test docstring as the lesson. Second, three genuine **discontinuities in the published equations** are now characterised rather than smoothed away: the FBP wind knee at 40 km/h (~0.04 % step *down*), the FWI BUI seam at 80 (~0.08 % step *down*), and — two orders of magnitude larger — the **GFWI intensity seam at FI = 100, a ~8.8 % step *down***, by far the largest in the ported system. All three are properties of the published science, not of this port; smoothing any of them would break parity. Third, ISI's monotonicity in FFMC holds with only a **~2.5× analytic margin**, so a single mistaken constant could silently invert eq 30's moisture function — and now cannot.

The lane then closed with a **whole-engine audit of its own defect class**: an AST screen flagged every `np.where` / `np.select` site whose discarded branch carries a partial-domain operation — fractional powers, logs, square roots, divisions; 29 sites across the engine — and per-site triage found **all 29 guarded**. The calm-wind grass-ROS pair had been the class's only live instance, and it was already fixed.
...
**Read that table strictly.** `inactive` means the profile, its source registry and its data-quality flags exist and are version-controlled — *not* that the region is served. Eleven regional profiles — ten Italian, plus Croatian Istria — are staged behind their fuel-input gates. The two Iberian/Aegean profiles are deliberately a **different kind**: `research_transferability` is not an operational kind, and neither is permitted `nowcast` or `scenario` in any state. Activating any profile is a gated promotion with its own evidence, not a configuration change.
